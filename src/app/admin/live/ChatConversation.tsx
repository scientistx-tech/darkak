...
import { useGetLiveMessagesQuery } from '@/redux/services/liveChatApis';
import { LiveMessage } from '@/types/apiTypes';
...
export default function ChatConversation({ selectedCustomer }: Props) {
  ...
  const { data, isLoading, refetch } = useGetLiveMessagesQuery(selectedCustomer.toString());
  ...
  useEffect(() => {
    if (data) {
      ...
      const apiMessages = Array.isArray(data) ? data : [data];

      const formattedMessages: Message[] = apiMessages.map((msg) => ({
        ...msg,
        status: 'seen', // or 'sent', depending on your logic
      }));
  ...
  const handleSendMessage = () => {
    ...
    const newMessage: Message = {
      id: Date.now(),
      message: input,
      senderId: user ? parseInt(user.id) : 1,
      isReadBy: null,
      ...
      <div ref={bottomRef} className="flex-grow space-y-3 overflow-y-auto bg-gray-50 px-6 py-4">
        {!selectedCustomer ? (
          ...
          messages.map((msg, i) => (
            <div
              key={i}
              className={`flex items-end gap-1 ${msg.senderId?.toString() == user?.id?.toString() ? 'justify-end' : 'justify-start'}`}
            >
              <div className="flex items-end gap-1">
                <div
                  className={`max-w-xs rounded-xl px-4 py-2 text-sm shadow ${msg.senderId?.toString() == user?.id?.toString()
