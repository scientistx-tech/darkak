...
const RefundDetails = () => {
  ...
  const handleAction = (action: "approved" | "rejected" | "refunded") => {
    setStatus(action);
    setOpen(true);
  };
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
                ...
                <div className="space-y-3 p-6">
                  <div>
                    ...
                    <span className="ml-2 font-medium">
                      {refundData?.vendor?.shopName}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">Email Address:</span>
                    <span className="ml-2 font-medium">
                      {refundData?.vendor?.email}
                    </span>
                  </div>
                  <div>
                    <span className="text-gray-600">Phone Number:</span>
                    <span className="ml-2 font-medium">
                      {refundData?.vendor?.phone}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
                ...
                <div className="p-6">
                  {!refundData?.deliveryInfo?.assigned ? (
                    <div className="py-8 text-center">
                      <div className="mx-auto mb-3 flex h-16 w-16 items-center justify-center rounded-full bg-gray-100">
                        <span className="text-2xl">👤</span>
                      </div>
                      <p className="text-gray-500">No delivery man assigned</p>
                    </div>
                  ) : (
                    <div>{/* Delivery man details would go here */}</div>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
              <div className="border-b border-gray-200 p-6">
                <h2 className="text-lg font-semibold text-gray-900">
                  Refund status changed log
                </h2>
              </div>
              <div className="p-6">
                {refundData?.statusLog?.length === 0 ? (
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      {/* <tbody>
                        {refundData?.statusLog?.map((log:any) => (
                ...
                <div className="p-4">
                  <div className="flex flex-col gap-3">
                    ...
                    {(refundData?.status === "approved" ||
                      refundData?.status === "pending") && (
                      <button
                        className="w-full rounded-md bg-teal-500 px-4 py-2 font-medium text-white hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2"
