...
export default function BagBrand() {
  ...
  const { data: brandList, refetch } = useGetBagBrandsQuery({});
  const [addBagBrand] = useAddBagBrandMutation();
  const [updateBagBrand] = useUpdateBagBrandMutation();
  const [deleteBagBrand] = useDeleteBagBrandMutation();
  ...
  const handleAddOrUpdate = async () => {
    ...
    try {
      if (editId) {
        await updateBagBrand({ id: editId, formData }).unwrap();
        ...
        await addBagBrand(formData).unwrap();
        toast.update(toastId, {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteBagBrand(id).unwrap();
      toast.update(toastId, {
  ...
  return (
    <div>
      <div className="mt-6 w-full rounded-lg bg-white p-6 shadow-md">
        <h2 className="mb-4 text-lg font-semibold">Add Bag Brand</h2>
