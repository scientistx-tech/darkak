...
export default function WatchSlider() {
  ...
  return (
    <div className="relative h-[calc(100vh-109px)] w-full overflow-hidden bg-primary">
      ...
      {/* Foreground Content */}
      <div className="relative z-10 ml-[5%] flex h-full w-[90%] items-center justify-center text-white">
        <div className="w-1/2">
          <p className="font-mono text-[60px]">{slides[currentIndex].title}</p>
          <p className="mt-[-10px] mb-5 font-serif text-[40px] font-thin text-white">
      ...
      {/* Dots / Controls */}
      <div className="absolute bottom-0 left-0 z-20 flex h-[50px] w-full items-center justify-center gap-4 bg-black/70 text-white">
        {slides.map((_, index) => (
          <button
            key={index}
            onClick={() => handleDotClick(index)}
            className={`h-[12px] w-[40px] rounded-full border transition-all duration-300 ${
              index === currentIndex
