...
import { useCreateDeliverySellerMutation } from "@/redux/services/seller/sellerCourierApis";
import { useUpdateOrderStatusSellerMutation } from "@/redux/services/seller/sellerOrderApis";
...
const CourierModal = ({
  ...
  const [createDelivery] = useCreateDeliverySellerMutation();
  const [changeOrderStatus] = useUpdateOrderStatusSellerMutation();
  ...
  useEffect(() => {
    if (orderDetails?.id) {
      fetch(
        `https://api.darkak.com.bd/api/seller/courier/cities/${selectedCourierId}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    if (cityId) {
      ...
      fetch(
        `https://api.darkak.com.bd/api/seller/courier/zones/${selectedCourierId}/${cityId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    if (zoneId) {
      ...
      fetch(
        `https://api.darkak.com.bd/api/seller/courier/area/${selectedCourierId}/${zoneId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
